...
def validate_data(*args):
    ...
    Args:
        *args (str): Worksheet names to validate.
        Use 'sales', 'scraps', or 'inventory' to specify the worksheets.
    ...
    if 'sales' in args:
        validate(sales_worksheet.get_all_values()[1:])
    if 'scraps' in args:
        validate(scrap_worksheet.get_all_values()[1:])
    if 'inventory' in args:
        validate(inv_worksheet.get_all_values()[1:])
...
def update_stock(sales=None, scraps=None, inventory_list=None):
    ...
    Args:
        sales (list of lists, optional):
        A list of lists representing the sales data,
        list items should contain GTIN and quantity sold.
        scraps (list of lists, optional):
        A list of lists representing the scraps data,
        list items should contain GTIN and quantity of scraps.
        inventory_list (list of lists, optional):
        A list of lists representing the inventory data,
        list items should contain GTIN and the quantity to update in stock.
    ...
    def update_qty(list_to_use):
        ...
        Args:
            list_to_use (list of lists):
            A list of lists representing sales or scraps data,
            list items should contain GTIN and quantity.

        Returns:
            bool: True if there are any errors during the update,
            False otherwise.
        """
        for product in list_to_use:
            # Find cell of product with correct gtin
            ...
            try:
                # Subtract sales and update the sheet
    ...
    if inventory_list is not None:
        for product in inventory_list:
            ...
            # Update sheet without subtraction for updateinv
...
def update():
    ...
    # Check if date already in history data
    if sales_history[-1]["date"] == str(datetime.now().date()):
        confirm_overwrite = input('''
        Todays date already exists in the sales history.
...
def updateinv():
    ...
    confirm = input('''
    Are you sure you want to update?
    This means overwriting your current stock data. (y/n) ''')
    if confirm == 'y':
...
def priceof(gtin):
    ...
    Returns:
        None: Prints price of product if the GTIN exists,
...
def instock(gtin):
    ...
    Returns:
        None: Prints stock quantity of product if the GTIN exists,
        otherwise returns None.
    """
    if not gtin_exists(gtin):
        print_output.print_error('gtin_not_exist')
...
def dataof(gtin):
    ...
    Returns:
        None: Prints data of the product if the GTIN exists,
        otherwise returns None.
    """
    if not gtin_exists(gtin):
        print_output.print_error('gtin_not_exist')
...
def help(command):
    ...
    Args:
        command (str):
        The command for which help information is to be displayed.
...
def execute_cmd(command):
    ...
    Returns:
        None: The function does not return any value.
        It performs actions based on the provided command.
    """

    # Define allowed commands and those which require additional parameters
    command_list = [
     'update', 'updateinv', 'priceof', 'instock',
     'dataof', 'scrap', 'help', 'exit',
     '1', '2', '3', '4', '5', '6', '7', '8']
    takes_arg = ['priceof', 'instock', 'dataof', 'help', '3', '4', '5', '7']

    # Split input into list [command, gtin/command]
    ...
    if len(command) == 0 or len(command[0]) < 1:
        ...
        if command[0] == 'help' or command[0] == '7':
            ...
            print(f'Invalid usage of {command[0]}: {command[0]} takes no parameters')
            print(f'Please type "help {command[0]}" to see correct usage.\n')
            return
        try:
